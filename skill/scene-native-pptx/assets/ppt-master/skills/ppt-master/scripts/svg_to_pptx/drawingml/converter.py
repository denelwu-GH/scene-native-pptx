...
def convert_g(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    group_name = _xml_escape(elem_id or f'Group {group_id}')
    group_description = _xml_escape(elem.get('data-role') or '')
    description_attr = f' descr="{group_description}"' if group_description else ''

    return ShapeResult(xml=f'''<p:grpSp>
<p:nvGrpSpPr>
<p:cNvPr id="{group_id}" name="{group_name}"{description_attr}/>
<p:cNvGrpSpPr/>
<p:nvPr/>
</p:nvGrpSpPr>
<p:grpSpPr>
