...
def _native_name(elem: ET.Element, fallback: str, shape_id: int) -> str:
    """Preserve semantic SVG names in PowerPoint's Selection Pane."""
    return elem.get('data-name') or elem.get('id') or f'{fallback} {shape_id}'
...
def convert_rect(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _native_name(elem, 'Rectangle', shape_id),
...
def convert_circle(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if _is_donut_circle(elem, ctx):
        ...
        return ShapeResult(
            xml=_wrap_shape(
                shape_id, _native_name(elem, 'Arc', shape_id),
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _native_name(elem, 'Ellipse', shape_id),
            off_x, off_y, ext_cx, ext_cy,
            geom, fill, stroke, effect, xfrm_attr=xfrm_attr,
        ),
        bounds_emu=bounds_emu,
...
def convert_line(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if has_marker:
        ...
        xml = _wrap_shape(
            shape_id, _native_name(elem, 'Line', shape_id),
...
{path_xml}
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _native_name(elem, 'Freeform', shape_id),
            off_x, off_y, w_emu, h_emu,
...
{path_xml}
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _native_name(elem, 'Polygon', shape_id),
            off_x, off_y, w_emu, h_emu,
            geom, fill, stroke,
        ),
        bounds_emu=(off_x, off_y, off_x + w_emu, off_y + h_emu),
...
{path_xml}
    ...
    return ShapeResult(
        xml=_wrap_shape(
            shape_id, _native_name(elem, 'Polyline', shape_id),
            off_x, off_y, w_emu, h_emu,
            geom, '<a:noFill/>', stroke,
        ),
        bounds_emu=(off_x, off_y, off_x + w_emu, off_y + h_emu),
