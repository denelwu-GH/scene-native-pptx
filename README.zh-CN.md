...
```text
使用 $scene-native-pptx，把下面的内容设计成一页精美的 16:9 可编辑 PowerPoint。
先建立设计契约并生成视觉参考稿，再通过 scene.json、受约束 SVG 和 DrawingML
完成原生重建，最后交付 PPTX 和完整 QA 证据。
```
...
测试套件包含一张全原生智能编排页，以及一张插画可独立替换的混合模式页面。
